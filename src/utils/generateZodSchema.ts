...
import { FieldConfig } from "@/components/common/DynamicForm/FormFields/FieldTypes";
  ...
  fields.forEach((field) => {
    ...
    switch (field.type) {
      ...
      case "file":
        ...
        validator = z.any().refine((val: any) => {
        ...
        if ("maxSize" in field && field.maxSize !== undefined) {
          ...
          validator = (validator as any).refine((val: any) => {
            if (!val) return true;
        ...
        if ("accept" in field && field.accept) {
          const acceptedTypes = field.accept.split(",").map((t: string) => t.trim().toLowerCase());
          validator = (validator as any).refine((val: any) => {
            if (!val) return true;
            const files = val instanceof FileList ? Array.from(val) : (Array.isArray(val) ? val : [val]);
